...
function runAllStaks(config = {}) {
	...
	if (config.stakEachFile) {
		config.source.forEach((filepath) => {
			...
			if (config.stakEachFile) {
				output = path.join(
					config.output,
					config.cwd ? path.relative(config.cwd, filepath) : path.basename(filepath)
				);
	...
	return Promise.all(bundlePromises).then((results) => {
		const result = {
			// result.config is original config with the resulting `content`.
			config: Object.assign(config, { content: results[results.length - 1].content }),
			// All results.
			all: results,
			// Watcher instance.
			watcher: undefined
		};
		// @todo  Add desired notifications.
		// Optionally watch the files.
		if (config.watch && !config.hasWatcher) {
			const chokidar = require('chokidar');
			result.watcher = chokidar.watch(config.source, {
				// persistent: true,
				// followSymlinks: true,
			...
			.reduce((promise, bundler, i) => {
				return promise.then((configResult) => {
					...
					if (typeof bundler.run === 'string') {
						bundler.run = config.cli
							? require(path.join(process.cwd(), bundler.run))
							: require(bundler.run);
					}
...
function normalizeConfig(config = {}) {
	...
	if (config.config) {
		...
		config = merge({}, cosmiconfig.load(process.cwd(), config.config).config, config);
	}
	...
	// If output is a directory, process source path files as separate staks. Otherwise process
	// source as a single stak.
	if (config.output && config.output[config.output.length - 1] === '/') {
		config.stakEachFile = true;
	} else if (typeof config.output === 'string') {
		const outputStats = fs.existsSync(config.output) && fs.statSync(config.output);
