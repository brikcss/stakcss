...
test("fails if `source` and `content` don't exist.", () => {
  return bundle({ output: './.temp' }).then((result) => {
    expect(result instanceof Error).toBe(true)
  })
})

test('fails if `bundlers` does not exist.', () => {
  return bundle({ content: 'Testing, testing...', output: './.temp' }).then((result) => {
    expect(result instanceof Error).toBe(true)
  })
})

test('fails if a bundler returns an invalid config object.', () => {
  return bundle({
...
test('fails if source paths do not exist', () => {
  return bundle({
...
test('runs all profiles with `config: <path>:all`', () => {
  return bundle({
...
test("returns content even if `output` doesn't exist.", () => {
  return bundle({
...
test('runs with each item in `bundlers` as an object.', () => {
  return bundle({
    source: 'test/fixtures/sample1/**/*',
    output: '.temp/one.md',
    bundlers: [
...
test('bundles `source` from a glob.', () => {
  return bundle({
    source: 'test/fixtures/sample1/**/*',
    output: '.temp/one.md',
    bundlers: [copyBundler]
...
test('bundles `source` from a glob and outputs separate files with `stakEachFile`.', () => {
  return bundle({
    source: 'test/fixtures/sample1/**/*',
    output: '.temp/test1',
    bundlers: [copyBundler],
...
test('bundles `source` from a directory and outputs separate files when `output` is a directory.', () => {
  return bundle({
    source: 'test/fixtures/sample1',
    output: '.temp/test1/',
    bundlers: [copyBundler]
...
test('outputs multiple files and renames them with `rename` callback.', () => {
  return bundle({
    source: 'test/fixtures/sample1',
    output: '.temp/test1/',
    rename (filepath) {
...
test('bundles `source` from an array of file paths and outputs as a single file when `output` is a filepath.', () => {
  return bundle({
...
test('bundles `source` array of files as single files if `output` is an existing directory.', () => {
...
test('bundles `source` files and outputs relative to `root`.', () => {
  return bundle({
    source: ['test/fixtures/sample1/sample.md', 'test/fixtures/sample1/sample.js'],
    output: '.temp/',
    root: 'test/fixtures',
...
test('bundles with `bundlers` as a comma-separated list and each bundler a node require', () => {
  return bundle({
    source: ['test/fixtures/sample1/sample.md', 'test/fixtures/sample1/sample.js'],
...
test('returns a watcher instance when run with `watch`', () => {
  return bundle({
    source: 'test/fixtures/sample1/**/*',
    output: '.temp/test1',
    bundlers: [copyBundler],
...
test('runs bundlers with node_modules', () => {
  return bundle({
    source: 'test/fixtures/sample1/**/*',
    output: '.temp/',
...
test('runs bundlers with node_modules shorthand require syntax', () => {
  return bundle({
    source: 'test/fixtures/sample1/**/*',
    output: '.temp/',
